...
class SSHAccessManager:
	...
	def block_ip(self, ip_address: str, reason: BlockReason,
    	...
    	Args:
        	...
        	expires_in: Optional expiration time in minutes
    	...
    	with self._iptables_lock():
        	if not self.is_blocked(ip_address):
            	...
            	if expires_in:
                	expires_at = expires_at.replace(minute=expires_at.minute + expires_in)
           	 
            	self._blocked_ips[ip_address] = BlockedIP(
                	ip_address=ip_address,
                	timestamp=datetime.now().replace(microsecond=0),
                	reason=reason,
                	expires_at=expires_at if expires_in else None,
                	notes=notes
            	)
